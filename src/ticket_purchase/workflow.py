...
## 如何从 XML 找元素
1. 搜索框: 找 resource-id 包含 "search" 的节点，复制完整的 resource-id 值
2. 按钮: 找 text="xxx" 的节点，使用 text 策略
3. 输入框: 找 class="android.widget.EditText" 的节点
4. 示例: <node resource-id="cn.damai:id/xxx" text="搜索" /> -> 用 resourceId="cn.damai:id/xxx" 或 text="搜索"

## 已知元素参考（优先使用这些）
- 首页搜索框: resourceId="cn.damai:id/homepage_header_search"
- 搜索输入框: className="android.widget.EditText"
- 购买按钮: text="立即购买" 或 text="立即预定" 或 text="选座购买"
- 确定按钮: text="确定"
- 提交按钮: text="立即提交" 或 text="提交订单"
- 关闭弹窗: text="知道了" 或 text="我知道了"

## 注意事项
- 从 XML 中找到实际存在的元素，不要猜测 resourceId
- 如果 resourceId 不确定，使用 text 或 textContains 定位
- 状态标签（如"预售"、"有票"）不可点击，要点击其左侧的场次日期或父容器
- 弹窗覆盖层在 XML 末尾，优先处理弹窗
- 如果元素不可见，尝试滑动查找
- 如果卡住超过 3 次，返回 failed
- 仔细查看 XML 中的 resource-id 属性，确保使用正确的值
