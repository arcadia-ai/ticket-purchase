...
1. 启动大麦 App（进入首页）
2. 点击搜索框，输入关键词，按回车搜索
3. **搜索结果出来后，点击第一个演出结果进入详情**（不要再按回车）
4. 如果需要选择城市，点击目标城市（如"长沙"）；如果已在详情页则跳过
5. 处理弹窗（如有"预选观演人"弹窗，点"知道了"跳过）
6. **点击购买按钮（显示为：立即预定、立即购买、立即预约、选座购买）**
7. 选择场次（选择显示"有票"或"预售"的场次）
...
## 关键说明
- **搜索结果页识别**：页面显示演出名称列表，每个演出下面有城市名（如"北京"、"长沙"）
- **搜索结果页操作**：直接点击(click)演出名称或目标城市名，进入演出详情页
...
## 搜索结果页操作原则
- **搜索结果的第一个通常就是目标演出，直接点击它**
- 不需要等待城市信息，直接点击演出名称/图片
- 如果点错了进入非目标页面，按返回键(key: back)重试
- **已经有搜索结果时，不要再按回车，要点击结果！**
...
- **搜索结果出来后，必须点击(click)演出条目进入详情页，不要再按回车！**
- 如果页面卡住，可以按返回键（key: back）重试

## 页面判断与操作对应（严格遵守）
- 首页（有搜索框图标）→ click 点击搜索框
- 搜索输入页（有输入框、光标）→ input 输入关键词，然后 key enter
- **搜索结果页（显示演出列表/关键词相关内容）→ click 点击第一个结果**
  - 判断依据：已输入关键词，页面有演出/艺人相关内容
  - 操作：click 点击，不是 key enter！
- 演出详情页（有"立即预定"等按钮）→ click 点击购买按钮
- 城市选择（显示多个城市）→ click 点击目标城市
- 场次选择页 → click 点击有票的场次
- 票档选择页 → click 点击有票的价格
